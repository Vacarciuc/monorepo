...
describe('authService', () => {
  ...
  describe('login', () => {
    ...
    it('saves token after successful login', async () => {
      const mockResponse: LoginResponse = { accessToken: 'test-token', role: 'CUSTOMER' as UserRole };
      vi.spyOn(authApi, 'login').mockResolvedValue(mockResponse);
      const saveTokenSpy = vi.spyOn(tokenService.tokenService, 'saveToken').mockImplementation(() => {});
      vi.spyOn(roleService.roleService, 'saveRole').mockImplementation(() => {});
    ...
    it('saves role after successful login', async () => {
      ...
      vi.spyOn(authApi, 'login').mockResolvedValue(mockResponse);
      vi.spyOn(tokenService.tokenService, 'saveToken').mockImplementation(() => {});
      const saveRoleSpy = vi.spyOn(roleService.roleService, 'saveRole').mockImplementation(() => {});
  ...
  describe('logout', () => {
    it('removes token and role', () => {
      const removeTokenSpy = vi.spyOn(tokenService.tokenService, 'removeToken').mockImplementation(() => {});
      const removeRoleSpy = vi.spyOn(roleService.roleService, 'removeRole').mockImplementation(() => {});
  ...
  describe('getAuthHeader', () => {
    it('returns authorization header when token exists', () => {
      vi.spyOn(tokenService.tokenService, 'getToken').mockReturnValue('test-token');
